...
export default function EditLessonPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const qArr = data.practice_questions ?? [];
      setQuestions(qArr.length ? qArr : [{ question: '', answer: '' }]);
      if (data.video_path) {
        setExistingVideoPath(data.video_path);
        const { data: pub } = supabase.storage.from('lesson-videos').getPublicUrl(data.video_path);
        setExistingVideoUrl(pub.publicUrl);
      }
      setLoading(false);
    })();
  }, [id]);
    ...
    <main style={{ flex: 1, padding: '40px clamp(20px,4vw,56px)', maxWidth: 760 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 28, gap: 16, flexWrap: 'wrap' }}>
        <div>
          ...
          <h1 style={{ fontSize: 26, margin: '8px 0 0' }}>Edit lesson</h1>
        </div>
      ...
      <div style={{ border: '1px solid var(--color-divider)', padding: 24, marginBottom: 24 }}>
        ...
        />
        {videoToShow && (
          <video src={videoToShow} controls style={{ width: '100%', marginTop: 12, aspectRatio: '16/9' }} />
        )}
      </div>
      ...
      {previewOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'var(--color-bg)', zIndex: 60, overflowY: 'auto' }}>
          ...
          <div style={{ maxWidth: 780, margin: '0 auto', padding: '40px clamp(20px,4vw,56px)' }}>
            ...
            {videoToShow ? (
              <video src={videoToShow} controls style={{ width: '100%', aspectRatio: '16/9', marginBottom: 32, border: '1px solid var(--color-divider)' }} />
            ) : (
